...
function IndexMediaShow({ mediaUrl, type, height, text, btnlist }: DataType) {
  ...
  return (
    <Box sx={{ position: "relative", height: "fit-content" }}>
      <IndexMediaShowBox height={height.desh}>
        {type === "image" ? (
          <Image
            src={mediaUrl.des}
            alt=""
      ...
      <IndexMediaShowBoxM height={height.moh}>
        {type === "image" ? (
          <Image
            src={mediaUrl.mo}
            alt=""
        ...
        >
          {btnlist.map((item) => (
            <Button
              ...
              size="xl"
              onClick={()=>router.push("/Product/M21210")}
            >
              {item.text}
            </Button>
          ))}
